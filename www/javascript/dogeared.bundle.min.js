...
function dogeared_notepad_build_list(){var a=$("#note-editor"),b=$("#notes-list"),c="";c+='<li><a href="#" id="note-add">Add a note</a></li>';store.forEach(function(d,e){console.log("b "+d);if(d.split("_")[0]=="notepad"){e.id||store.remove(d);if(e.deleted)console.log(d+" is deleted");else{if(!e.source_id){var f=dogeared_notepad_source();e.source_id=f;store.set(d,e)}console.log(e);var j=e.title;f=e.source_id;c+="<li>";c+='<a href="#" class="note-item" data-note-key="'+d+'">'+j+"</a> ";c+="<small>"+
f+"</small>";c+="</li>"}}});b.html(c);$("#note-add").click(function(){dogeared_notepad_add_note();return false});$(".note-item").click(function(){var d=$(this).attr("data-note-key");dogeared_notepad_load_note(d);return false});a.hide();b.show()}
function dogeared_notepad_sync_list(){store.forEach(function(b,c){if(b.split("_")[0]=="notepad")c["delete"]?store.remove(b):dogeared_notepad_sync_note(c)});var a=setInterval(function(){if(notepad_inprocess_sync.length)dogeared_feedback("Synching in progress");else{dogeared_feedback_ok("Syncing complete",5);clearInterval(a);dogeared_notepad_get_list()}},1E3)}
function dogeared_notepad_get_list(){dogeared_api_call("dogeared.notepad.getList",{},function(a){a=a.notes.note;for(var b=a.length,c=0;c<b;c++){var d=a[c],e=dogeared_notepad_key(d);console.log("k "+e);if(!store.get(e)){console.log("add "+e);store.set(e,d)}}dogeared_notepad_build_list()},function(){})}function dogeared_notepad_sync_pending_notes(a){for(id in a){var b=dogeared_notepad_key({id:id});b=store.get(b);dogeared_notepad_sync_note(b)}}
