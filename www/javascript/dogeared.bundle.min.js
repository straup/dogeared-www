function dogeared_init(){window.addEventListener("offline",dogeared_on_offline);window.addEventListener("online",dogeared_on_online);dogeared_network_init();dogeared_appcache_init();dogeared_highlights_init();dogeared_cache_highlights_init()}function dogeared_abs_root_url(){return document.body.getAttribute("data-abs-root-url")}function dogeared_now(){var a=(new Date).getTime();return a=parseInt(a/1E3)}
...
false);window.applicationCache.addEventListener("progress",dogeared_appcache_on_event,false);window.applicationCache.addEventListener("updateready",dogeared_appcache_on_updateready,false)}
function dogeared_appcache_on_event(a){a=a.type;console.log("appcache event "+a);if(a=="checking")dogeared_appcache_progress=0;else if(a=="progress"){dogeared_appcache_progress+=1;dogeared_network_is_online()&&dogeared_feedback("checking for updates ("+dogeared_appcache_progress+")")}else dogeared_feedback_reset()}function dogeared_appcache_on_error(a){console.log("appcache error");console.log(a)}
function dogeared_appcache_on_updateready(){if(window.applicationCache.status==window.applicationCache.UPDATEREADY)$("#document").html()||window.location.reload()}function dogeared_appcache_status_code(){return window.applicationCache.status}
...
function dogeared_feedback_reset(){$("#feedback-general").html("")};function dogeared_api_call(a,b,c,d){var e=dogeared_api_endpoint();b.method=a;if(!b.access_token)if(a=dogeared_api_site_token())b.access_token=a;$.ajax({url:e,type:"POST",data:b,dataType:"json",success:function(j){c&&c(j)},error:function(j){if(j=function(g){if(g.responseText)try{return g=JSON.parse(g.responseText)}catch(k){console.log("Failed to parse response text");dogeared_feedback_error("The API is full of crazy-talk: Failed to parse response text")}else{console.log("Missing response text");console.log(g);
dogeared_feedback_error("The API is full of crazy-talk: Missing response text")}}(j))j.error?dogeared_feedback_error("The API is sad: "+j.error.message):dogeared_feedback(j);d&&d(j)}})}function dogeared_api_endpoint(){return document.body.getAttribute("data-api-endpoint")}function dogeared_api_site_token(){return document.body.getAttribute("data-api-site-token")};function dogeared_extruder_init(){window.addEventListener("offline",function(a){dogeared_extruder_on_offline(a)});window.addEventListener("online",function(a){dogeared_extruder_on_online(a)});$("#fetch").submit(function(){var a=$("#url");a=a.val();dogeared_api_call("dogeared.documents.addDocument",{url:a},function(b){var c=b.document.id;c='Okay! <a href="'+(dogeared_abs_root_url()+"documents/"+c+"/")+'">That document</a> is ready to be dogeared.';dogeared_feedback_ok(c,60);$("#button-dogear").removeAttr("disabled");
dogeared_cache_documents_store(b.document)});dogeared_feedback("Fetching your document, now...");$("#button-dogear").attr("disabled","disabled");return false});dogeared_network_is_online()||dogeared_extruder_hide_form()}function dogeared_extruder_on_online(){dogeared_extruder_show_form()}function dogeared_extruder_on_offline(){dogeared_extruder_hide_form()}function dogeared_extruder_show_form(){$("#fetch").show()}function dogeared_extruder_hide_form(){$("#fetch").hide()};function dogeared_documents_init(){dogeared_document_init_highlight_controls();window.addEventListener("offline",function(b){dogeared_documents_on_offline(b)});window.addEventListener("online",function(b){dogeared_documents_on_online(b)});$("#dogeared-navi li a").click(function(){if(!dogeared_network_is_online()){$(this).html()=="reading list"&&dogeared_documents_load_cache();return false}});$(".delete-document").click(function(){if(!confirm("Are you sure you want to remove this document?"))return false;
var b=$(this).attr("data-document-id");dogeared_api_call("dogeared.documents.deleteDocument",{document_id:b},function(){$("#document-"+b).remove();store.remove("dogeared_"+b);dogeared_feedback("Okay, that document has been removed from your reading list")})});if(dogeared_network_is_online())try{dogeared_documents_fill_cache();dogeared_documents_load_cache()}catch(a){dogeared_feedback_error("Oh no! There was a problem loading your documents: "+a);console.log(a)}else dogeared_documents_load_cache()}
function dogeared_documents_on_online(){console.log("documents: online")}function dogeared_documents_on_offline(){console.log("documents: offline");dogeared_documents_load_cache()}
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents(),b=a.length;if(b==0)return false;a=dogeared_documents_sort(a);$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var j=a[e],g=j.id,k=j.display_title;if(k==undefined)k="Unknown title #"+g;d[g]=e;var n=dogeared_abs_root_url();n+="documents/"+g+"/";var h='<div class="row excerpt">';h+='<h3><a href="';h+=n;h+='" class="load-doc" data-document-id="';
h+=htmlspecialchars(g);h+='">';h+=htmlspecialchars(k);h+="</a>";h+=" <pre><small>"+htmlspecialchars(j.url)+"</small></pre>";h+="</h3>";h+="<div>";h+='<button class="btn btn-sm delete-document pull-right" data-document-id="';h+=g;h+='" data-button-action="delete">Delete</button>';h+="</div>";h+="</div>";c.append(h)}$(".load-doc").click(function(){dogeared_network_is_online();var l=$(this).attr("data-document-id"),m=a[d[l]],o=m.display_title;if(o==undefined)o="Unknown title #"+l;var p=JSON.parse(m.body);
m=htmlspecialchars(m.url);var f='<div class="document" id="document"';f+=' data-document-id="';f+=htmlspecialchars(l);f+='">';f+='<div class="row">';f+="<h3>";f+=htmlspecialchars(o);f+="</h3>";f+=" <pre><small>"+m+"</small></pre>";f+="</div>";f+='<div class="row">';for(var q in p){f+="<p>";f+=htmlspecialchars(p[q]);f+="</p>"}f+="</div>";f+='<div class="row">';f+=" <pre><small>"+m+"</small></pre>";f+="</div>";f+="<div>";f+='<a href="">reading list</a>';f+="</div>";f+="</div>";$(".excerpt").remove();
c.append(f);return false});dogeared_document_init()}function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}
function dogeared_documents_fill_cache(){for(var a={},b=dogeared_cache_documents(),c=b.length,d=0;d<c;d++)a[b[d].id]=1;dogeared_feedback("Fetching documents");dogeared_api_call("dogeared.documents.getList",{},function(e){dogeared_feedback("Storing documents");e=e.documents.document;var j=e.length;dogeared_documents_store_docs(e);dogeared_feedback("Cleaning up document cache");for(var g=0;g<j;g++){var k=e[g].document_id;a[k]&&delete a[k]}for(k in a){e="dogeared_"+k;console.log("delete "+e);store.remove(e)}dogeared_feedback_reset()})}
function dogeared_documents_sort(a){a=usort(a,function(b,c){if(b.created==c.created)return 0;return b.created>c.created?-1:1});return a=array_values(a)};var current_selection="";
function dogeared_document_init(){dogeared_document_init_highlight_controls();$(".delete-document").click(function(){if(!dogeared_network_is_online()){alert("Offline deletes are not supported yet.");return false}if(!confirm("Are you sure you want to remove this document?"))return false;var a=$(this).attr("data-document-id");dogeared_api_call("dogeared.documents.deleteDocument",{document_id:a},function(){store.remove("dogeared_"+a);var b=dogeared_abs_root_url()+"documents/";location.href=b})})}
function dogeared_document_init_highlight_controls(){if(typeof window.ontouchend=="object"){console.log("enable touch based highlight controls");$(document).bind("selectionchange",dogeared_document_selected_selectionchange)}else $(document).bind("mouseup",dogeared_document_selected_mouseup)}
function dogeared_document_selected_mouseup(a){console.log("on mouseup");a=a.target;if(a.nodeName=="BUTTON"){a=$(a);a.attr("data-button-action")!="delete"&&dogeared_document_add_highlight()}else{$(".highlight").remove();var b=window.getSelection();if(b.toString()!=""){range=window.getSelection().getRangeAt(0);expandedSelRange=range.cloneRange();range.collapse(false);a=document.createElement("div");a.innerHTML='<span class="highlight"> <button id="highlight" class="btn btn-sm">Highlight</button> </span>';
for(var c=document.createDocumentFragment(),d,e;d=a.firstChild;)e=c.appendChild(d);range.insertNode(c);if(e){expandedSelRange.setEndAfter(e.previousSibling);b.removeAllRanges();b.addRange(expandedSelRange)}}}}
function dogeared_document_selected_selectionchange(){console.log("on selectionchange");$(".highlight").remove();var a=window.getSelection().toString();console.log("text is "+a);if(a!=""){current_selection=a;range=window.getSelection().getRangeAt(0);expandedSelRange=range.cloneRange();range.collapse(false);a=document.createElement("div");a.innerHTML='<span class="highlight"> <button id="highlight" class="btn btn-sm">Highlight</button> </span>';for(var b=document.createDocumentFragment(),c;c=a.firstChild;)b.appendChild(c);
range.insertNode(b);console.log("do highlight");$("#highlight").click(dogeared_document_add_highlight)}}
...
dogeared_cache_highlights_status()}};function dogeared_highlights_init(){console.log("init highlights");window.addEventListener("online",function(a){console.log("online highlights");setTimeout(function(){dogeared_highlights_on_online(a)},3E3)})}
function dogeared_highlights_on_online(){var a=dogeared_cache_highlights(),b=a.length;console.log(b+" pending highlights");for(var c=0;c<b;c++){var d=a[c],e=d.cache_key;d={document_id:d.document_id,text:d.text,created:d.created};console.log(d);dogeared_api_call("dogeared.highlights.addHighlight",d,function(){console.log("remove "+e);store.remove(e)})}}
...
function dogeared_notepad_load_note(a){var b=store.get(a),c=$("#notes-list"),d=$("#note-editor"),e=$("#note-editor-close"),j=$("#note-editor-delete");d.attr("data-note-id",a);$("#note-title").html(b.title);$("#note-body").html(b.body);e.click(function(){dogeared_notepad_close_note();return false});j.click(function(){if(!confirm("Are you sure you want to delete this note?"))return false;dogeared_notepad_delete_note();dogeared_notepad_build_list();return false});c.hide();d.show();notepad_interval=setInterval(function(){dogeared_notepad_save_note()},
...
function dogeared_notepad_build_list(){var a=[];store.forEach(function(h,l){if(h.split("_")[0]=="notepad"){l.id||store.remove(h);parseInt(l.deleted)||a.push(l)}});a=dogeared_notepad_sort(a);for(var b=$("#note-editor"),c=$("#notes-list"),d="",e=a.length,j=0;j<e;j++){var g=a[j],k="notepad_"+g.id,n=g.title;g=g.source_id;d+="<li>";d+='<a href="#" class="note-item" data-note-key="'+k+'">'+n+"</a> ";d+=parseInt(g)?"<small>device id #"+g+"</small>":"<small>unknown source</small>";d+="</li>"}d+='<li><a href="#" id="note-add">Add a note</a></li>';
