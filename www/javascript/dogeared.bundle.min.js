...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents(),b=a.length;if(b==0)return false;a=dogeared_documents_sort(a);$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var h=a[e],j=h.id,k=h.display_title;if(k==undefined)k="Unknown title #"+j;d[j]=e;var n=dogeared_abs_root_url();n+="documents/"+j+"/";var g='<div class="row excerpt">';g+='<h3><a href="';g+=n;g+='" class="load-doc" data-document-id="';
g+=htmlspecialchars(j);g+='">';g+=htmlspecialchars(k);g+="</a>";g+=" <pre><small>"+htmlspecialchars(h.url)+"</small></pre>";g+="</h3>";g+="<div>";g+='<button class="btn btn-sm delete-document pull-right" data-document-id="';g+=j;g+='" data-button-action="delete">Delete</button>';g+="</div>";g+="</div>";c.append(g)}$(".load-doc").click(function(){dogeared_network_is_online();var l=$(this).attr("data-document-id"),m=a[d[l]],o=m.display_title;if(o==undefined)o="Unknown title #"+l;var p=JSON.parse(m.body);
...
function dogeared_documents_fill_cache(){for(var a={},b=dogeared_cache_documents(),c=b.length,d=0;d<c;d++)a[b[d].id]=1;dogeared_feedback("Fetching documents");dogeared_api_call("dogeared.documents.getList",{},function(e){dogeared_feedback("Storing documents");e=e.documents.document;var h=e.length;dogeared_documents_store_docs(e);dogeared_feedback("Cleaning up document cache");for(var j=0;j<h;j++){var k=e[j].document_id;a[k]&&delete a[k]}for(k in a){e="dogeared_"+k;console.log("delete "+e);store.remove(e)}dogeared_feedback_reset()})}
...
dogeared_cache_highlights_status()}};function dogeared_highlights_init(){}function dogeared_highlights_init_list(){$(".delete-highlight").click(function(){if(!confirm("Are you sure you want to delete this highlight?"))return false;var a=$(this).attr("data-highlight-id");dogeared_api_call("dogeared.highlights.deleteHighlight",{id:a},function(){$("#highlight-"+a).remove();dogeared_feedback("That highlight has been deleted.")})})}
...
function dogeared_notepad_load_note(a){var b=store.get(a),c=$("#notes-list"),d=$("#note-editor"),e=$("#note-editor-close"),h=$("#note-editor-delete");d.attr("data-note-id",a);$("#note-title").html(b.title);$("#note-body").html(b.body);e.click(function(){dogeared_notepad_close_note();return false});h.click(function(){if(!confirm("Are you sure you want to delete this note?"))return false;dogeared_notepad_delete_note();dogeared_notepad_build_list();return false});c.hide();d.show();notepad_interval=setInterval(function(){dogeared_notepad_save_note()},
...
function dogeared_notepad_build_list(){var a=[];store.forEach(function(g,l){if(g.split("_")[0]=="notepad"){l.id||store.remove(g);parseInt(l.deleted)||a.push(l)}});a=dogeared_notepad_sort(a);for(var b=$("#note-editor"),c=$("#notes-list"),d="",e=a.length,h=0;h<e;h++){var j=a[h],k="notepad_"+j.id,n=j.title;j=j.source_id;d+="<li>";d+='<a href="#" class="note-item" data-note-key="'+k+'">'+n+"</a> ";d+=parseInt(j)?"<small>device id #"+j+"</small>":"<small>unknown source</small>";d+="</li>"}d+='<li><a href="#" id="note-add">Add a note</a></li>';
c.html(d);$("#note-add").click(function(){dogeared_notepad_add_note();return false});$(".note-item").click(function(){var g=$(this).attr("data-note-key");dogeared_notepad_load_note(g);return false});b.hide();c.show()}
