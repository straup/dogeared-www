...
function dogeared_notepad_add_note(){var a=Math.uuid(),b=dogeared_now(),c=dogeared_notepad_source();a={id:a,source_id:c,created:b,lastmodified:0,deleted:0,title:"Untitled Note #"+a,body:"This note left intentionally blank."};b=dogeared_notepad_key(a);store.set(b,a);dogeared_notepad_load_note(b)}
...
function dogeared_notepad_build_list(){var a=$("#note-editor"),b=$("#notes-list"),c="";c+='<li><a href="#" id="note-add">Add a note</a></li>';store.forEach(function(d,e){if(d.split("_")[0]=="notepad"){e.id||store.remove(d);if(!e.deleted){if(!e.source_id){var f=dogeared_notepad_source();e.source_id=f;store.set(d,e)}var j=e.title;f=e.source_id;c+="<li>";c+='<a href="#" class="note-item" data-note-key="'+d+'">'+j+"</a> ";c+="<small>"+f+"</small>";c+="</li>"}}});b.html(c);$("#note-add").click(function(){dogeared_notepad_add_note();
return false});$(".note-item").click(function(){var d=$(this).attr("data-note-key");dogeared_notepad_load_note(d);return false});a.hide();b.show()}function dogeared_notepad_sync_list(){store.forEach(function(a,b){a.split("_")[0]=="notepad"&&dogeared_notepad_sync_note(b)})}function dogeared_notepad_sync_pending_notes(a){for(id in a){var b=dogeared_notepad_key({id:id});b=store.get(b);dogeared_notepad_sync_note(b)}}
