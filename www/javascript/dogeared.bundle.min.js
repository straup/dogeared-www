...
dogeared_appcache_on_event,false);window.applicationCache.addEventListener("progress",dogeared_appcache_on_event,false);window.applicationCache.addEventListener("updateready",dogeared_appcache_on_updateready,false)}function dogeared_appcache_on_event(a){a=a.type;console.log("appcache event "+a);if(a=="progress")dogeared_network_is_online()&&dogeared_feedback("checking for updates");else dogeared_feedback_reset()}function dogeared_appcache_on_error(a){console.log("appcache error");console.log(a)}
...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents();a=dogeared_documents_sort(a);var b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var f=a[e],j=f.id,l=f.display_title;if(l==undefined)l="Unknown title #"+j;d[j]=e;var g='<div class="row excerpt">';g+='<h3><a href="#';g+='" class="load-doc" data-document-id="';g+=htmlspecialchars(j);g+='">';
g+=htmlspecialchars(l);g+="</a>";g+=" <small>offline cache</small>";g+="</h3>";g+="<p>";g+=htmlspecialchars(f.excerpt);g+=' <a href="#" class="load-doc" data-document-id="';g+=htmlspecialchars(j);g+='">...</a></p>';g+="</div>";c.append(g)}$(".load-doc").click(function(){var m=$(this).attr("data-document-id"),k=a[d[m]],n=k.display_title;if(n==undefined)n="Unknown title #"+m;k=JSON.parse(k.body);var h='<div class="row document" id="document"';h+=' data-document-id="';h+=htmlspecialchars(m);h+='">';
h+="<h3>";h+=htmlspecialchars(n);h+="<small>offline cache</small>";h+="</h3>";for(var o in k){h+="<p>";h+=htmlspecialchars(k[o]);h+="</p>"}h+="</document>";$(".excerpt").remove();c.append(h);return false});dogeared_document_init()}function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}
function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}function dogeared_documents_fill_cache(){dogeared_api_call("dogeared.documents.getList",{},function(a){dogeared_documents_store_docs(a.documents.document)})}function dogeared_documents_sort(a){a=usort(a,function(b,c){if(b.created==c.created)return 0;return b.created<c.created?-1:1});return a=array_values(a)};var current_selection="";function dogeared_document_init(){dogeared_document_init_highlight_controls();$(".delete-document").click(function(){if(!confirm("Are you sure you want to remove this document?"))return false;var a={document_id:$(this).attr("data-document-id")};dogeared_api_call("dogeared.documents.deleteDocument",a,function(){var b=dogeared_abs_root_url()+"documents/?deleted=1";location.href=b})})}
...
function dogeared_document_selected_mouseup(a){console.log("on mouseup");if(a.target.nodeName=="BUTTON")dogeared_document_add_highlight();else{$(".highlight").remove();a=window.getSelection();if(a.toString()!=""){range=window.getSelection().getRangeAt(0);expandedSelRange=range.cloneRange();range.collapse(false);var b=document.createElement("div");b.innerHTML='<span class="highlight"> <button id="highlight" class="btn btn-sm">Highlight</button> </span>';for(var c=document.createDocumentFragment(),
d,e;d=b.firstChild;)e=c.appendChild(d);range.insertNode(c);if(e){expandedSelRange.setEndAfter(e.previousSibling);a.removeAllRanges();a.addRange(expandedSelRange)}}}}
...
function dogeared_notepad_build_list(){var a=$("#note-editor"),b=$("#notes-list"),c="";c+='<li><a href="#" id="note-add">Add a note</a></li>';store.forEach(function(d,e){if(d.split("_")[0]=="notepad"){e.id||store.remove(d);if(!parseInt(e.deleted)){var f=e.title,j=e.source_id;c+="<li>";c+='<a href="#" class="note-item" data-note-key="'+d+'">'+f+"</a> ";c+=parseInt(j)?"<small>device id #"+j+"</small>":"<small>unknown source</small>";c+="</li>"}}});b.html(c);$("#note-add").click(function(){dogeared_notepad_add_note();
