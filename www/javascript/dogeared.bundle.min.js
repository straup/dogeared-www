function dogeared_init(){window.addEventListener("offline",dogeared_on_offline);window.addEventListener("online",dogeared_on_online);$("a").click(function(){var a=$(this);if(!dogeared_network_is_online()){a.attr("id")=="reading list"&&dogeared_documents_load_cache();return false}});dogeared_network_init();dogeared_omgwtf_init();dogeared_appcache_init();dogeared_highlights_init();dogeared_cache_highlights_init();dogeared_whosonfirst_init();dogeared_init_touch_controls()}
...
function dogeared_now(){var a=(new Date).getTime();return a=parseInt(a/1E3)}function dogeared_on_online(a){dogeared_omgwtf("came online at "+a.timeStamp);dogeared_feedback("You appear to be online again.");$(".appcache_equals_no").show()}function dogeared_on_offline(a){dogeared_omgwtf("went offline at "+a.timeStamp);dogeared_feedback("You appear to be offline.");$(".appcache_equals_no").hide()}
function dogeared_whosonfirst_init(){dogeared_omgwtf("whosonfirst init");var a=dogeared_whosonfirst_key();store.get(a)||store.set(a,{})}function dogeared_whosonfirst_key(){return"dogeared_whosonfirst"}function dogeared_whosonfirst_get(a){var b=dogeared_whosonfirst_key();return store.get(b)[a]}function dogeared_whosonfirst_set(a,b){var c=dogeared_whosonfirst_key(),d=store.get(c);d[a]=b;store.set(c,d)};function dogeared_network_init(){window.addEventListener("offline",dogeared_network_on_offline);window.addEventListener("online",dogeared_network_on_online);dogeared_network_status()}function dogeared_network_is_online(){return navigator.onLine}function dogeared_network_on_online(){dogeared_network_status();$(".btn-save-highlight").removeAttr("disabled")}function dogeared_network_on_offline(){dogeared_network_status();$(".btn-save-highlight").attr("disabled","disabled")}
function dogeared_network_status(){var a=$("#feedback-network-status");if(dogeared_network_is_online()){dogeared_network_hints_online();a.html("you are awake and connected to the network")}else{dogeared_network_hints_offline();a.html("you are playing hide-and-go-seek with the sky and the sky is winning.")}}function dogeared_network_hints_online(){$("#ima-online").show();$("#ima-offline").hide()}function dogeared_network_hints_offline(){$("#ima-online").hide();$("#ima-offline").show()};function dogeared_omgwtf(a,b){var c=new Date;c=c.toISOString();c="["+c+"] "+a;console.log(c);b&&dogeared_omgwtf_append(c)}function dogeared_omgwtf_init(){dogeared_omgwtf_purge()}function dogeared_omgwtf_append(a){var b=store.get("omgwtf");b.push(a);store.set("omgwtf",b)}function dogeared_omgwtf_purge(){var a=store.get("omgwtf");a=[];store.set("omgwtf",a)};var dogeared_appcache_progress=0;
...
function dogeared_appcache_status(){switch(dogeared_appcache_status_code()){case window.applicationCache.UNCACHED:return"UNCACHED";case window.applicationCache.IDLE:return"IDLE";case window.applicationCache.CHECKING:return"CHECKING";case window.applicationCache.DOWNLOADING:return"DOWNLOADING";case window.applicationCache.UPDATEREADY:return"UPDATEREADY";case window.applicationCache.OBSOLETE:return"OBSOLETE";default:return"UKNOWN CACHE STATUS"}};var de_feedback_timeout=null;function dogeared_feedback_ok(a,b){dogeared_feedback(a);b||(b=10);b*=1E3;de_feedback_timeout&&clearTimeout(de_feedback_timeout);de_feedback_timeout=setTimeout(function(){dogeared_feedback_reset()},b)}function dogeared_feedback_error(a){$("#warning").show();var b=$("#feedback-warning");b.html(a+"<br />"+b.html());b.show();dogeared_omgwtf("error: "+a);dogeared_feedback(a)}function dogeared_feedback_api_error(a){dogeared_feeback_error(a.error.message)}
function dogeared_feedback_modal(a){dogeared_feedback_ok(a,10);alert(a)}function dogeared_feedback(a){$("#feedback-general").html(a)}function dogeared_feedback_reset(){$("#feedback-general").html("")};function dogeared_api_call(a,b,c,d){var e=b.append?1:0;if(e){b.append("method",a);b.access_token||b.append("access_token",dogeared_api_site_token())}else{b.method=a;b.access_token||(b.access_token=dogeared_api_site_token())}b={url:dogeared_api_endpoint(),type:"POST",data:b,dataType:"json",success:function(f){dogeared_api_hints_ok();c&&c(f)},error:function(f){dogeared_omgwtf("[error] XHR error calling: "+a);dogeared_api_hints_error();if(f=function(g){if(g.responseText)try{return g=JSON.parse(g.responseText)}catch(j){console.log("Failed to parse response text");
dogeared_feedback_error("The API is full of crazy-talk: Failed to parse response text")}else{console.log("Missing response text");console.log(g);dogeared_feedback_error("The API is full of crazy-talk: Missing response text")}}(f))f.error?dogeared_feedback_error("The API is sad: "+f.error.message):dogeared_feedback(f);d&&d(f)}};if(e){b.cache=false;b.contentType=false;b.processData=false}$.ajax(b);dogeared_api_hints_dispatch()}
function dogeared_api_endpoint(){return document.body.getAttribute("data-api-endpoint")}function dogeared_api_site_token(){return document.body.getAttribute("data-api-site-token")}function dogeared_api_hints_ok(){$("#api-dispatch").hide();$("#api-ok").show();setTimeout(function(){$("#api-ok").hide()},5E3)}function dogeared_api_hints_error(){$("#api-dispatch").hide();$("#api-error").show()}function dogeared_api_hints_dispatch(){$("#api-dispatch").show()};function dogeared_extruder_init(){window.addEventListener("offline",function(a){dogeared_extruder_on_offline(a)});window.addEventListener("online",function(a){dogeared_extruder_on_online(a)});$("#fetch").submit(function(){var a=$("#url");a=a.val();dogeared_api_call("dogeared.documents.addDocument",{url:a},function(b){var c=b.document.id;c='Okay! <a href="'+(dogeared_abs_root_url()+"documents/"+c+"/")+'">That document</a> is ready to be dogeared.';dogeared_feedback_ok(c,60);$("#button-dogear").removeAttr("disabled");
dogeared_cache_documents_store(b.document);$("#url").val("")});dogeared_feedback("Fetching your document, now...");$("#button-dogear").attr("disabled","disabled");return false});dogeared_network_is_online()||dogeared_extruder_hide_form()}function dogeared_extruder_on_online(){dogeared_extruder_show_form()}function dogeared_extruder_on_offline(){dogeared_extruder_hide_form()}function dogeared_extruder_show_form(){$("#fetch").show()}function dogeared_extruder_hide_form(){$("#fetch").hide()};var dogeared_fill_cache_count=0;function dogeared_documents_init(){window.addEventListener("offline",function(a){dogeared_documents_on_offline(a)});window.addEventListener("online",function(a){dogeared_documents_on_online(a)});dogeared_documents_init_keyboard();dogeared_documents_load_index();dogeared_documents_process_to_delete()}function dogeared_documents_init_keyboard(){$(document).keydown(function(a){a.keyCode==70&&a.shiftKey&&screenfull.request()})}
function dogeared_documents_on_online(){dogeared_omgwtf("documents: online");dogeared_documents_process_to_delete()}function dogeared_documents_on_offline(){dogeared_omgwtf("documents: offline");dogeared_documents_currently_reading()||dogeared_documents_load_cache()}function dogeared_documents_process_to_delete(){dogeared_cache_process_pending_delete()}
function dogeared_documents_load_index(){if(dogeared_network_is_online()){dogeared_documents_load_cache();try{dogeared_documents_fill_cache(function(){var b=function(){dogeared_omgwtf("checking fill count: "+dogeared_fill_cache_count);if(dogeared_fill_cache_count){dogeared_feedback("Loading documents, "+dogeared_fill_cache_count+" left");setTimeout(b,500)}else dogeared_documents_load_cache()};b()})}catch(a){dogeared_feedback_error("Oh no! There was a problem loading your documents: "+a)}}else dogeared_documents_load_cache();
dogeared_document_unbind_highlight_controls()}
function dogeared_documents_init_delete_controls(){dogeared_omgwtf("init documents delete controls");var a=function(b){if(!confirm("Are you sure you want to remove this document?"))return false;var c=b.attr("data-document-id");if(!c){alert("Unable to determine document ID");return false}dogeared_omgwtf("delete document "+c);if(!dogeared_network_is_online()){b=$("#dogeared-document-"+c);b.remove();dogeared_cache_schedule_pending_delete(c);return false}dogeared_api_call("dogeared.documents.deleteDocument",{document_id:c},
function(){dogeared_cache_remove_document(c)});b=$("#dogeared-document-"+c);b.remove()};$(".delete-document").click(function(){dogeared_omgwtf("click to delete");try{var b=$(this);a(b)}catch(c){dogeared_feedback_error("Huh. Deleting your document failed because: "+c)}})}
function dogeared_documents_load_cache(){dogeared_omgwtf("load documents cache");dogeared_feedback_reset();window.scrollTo(0,0);var a=store.get("dogeared_to_delete"),b=dogeared_documents_cache(),c=b.length;if(c==0)return false;b=dogeared_documents_sort(b);$(".excerpt").remove();$(".document").remove();var d=$("#documents");dogeared_abs_root_url();for(var e=0;e<c;e++){var f=b[e],g=f.id;if(g)if(a&&a[g])dogeared_omgwtf("skipping document "+g+" because it is set to be deleted");else{var j=f.display_title;
...
h+=g;h+='" data-button-action="delete">Delete</button>';h+="</div>";h+="</div>";d.append(h)}}$(".load-doc").click(function(){try{dogeared_network_is_online();var l=$(this).attr("data-document-id");dogeared_documents_load_doc(l)}catch(m){dogeared_feedback("Failed to load document because "+m);console.log(m)}return false});dogeared_documents_init_delete_controls();dogeared_document_init()}
function dogeared_documents_get_text(a){var b=a.document_id;if(b=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:b,lastmodified:a.lastmodified},function(c){if(c){var d=c.document;dogeared_cache_store_document(d,function(){dogeared_omgwtf("store "+d.display_title+" ("+d.id+") "+ok);dogeared_documents_fill_cache_decr()})}else dogeared_documents_fill_cache_decr()},function(){dogeared_documents_fill_cache_decr()});dogeared_documents_fill_cache_incr()}
...
function dogeared_documents_fill_cache(a){for(var b={},c=dogeared_documents_cache(),d=c.length,e=0;e<d;e++)b[c[e].id]=c[e].lastmodified;dogeared_feedback("Fetching documents");dogeared_api_call("dogeared.documents.getList",{},function(f){dogeared_feedback("Storing documents");f=f.documents.document;var g=f.length;dogeared_documents_store_docs(f);dogeared_feedback("Cleaning up document cache");for(var j=0;j<g;j++){var k=f[j].document_id;if(b[k]){dogeared_omgwtf("remove "+k+" from purge cache");delete b[k]}}for(k in b){f=
"dogeared_"+k;dogeared_omgwtf("remove "+f+" from list");store.remove(f)}dogeared_feedback_reset();a&&a()},function(){})}function dogeared_documents_sort(a){a=usort(a,function(b,c){if(b.created==c.created)return 0;return b.created>c.created?-1:1});return a=array_values(a)}function dogeared_documents_currently_reading(){return $("#document").html()?1:0}
function dogeared_documents_load_doc(a){var b=store.get("dogeared_"+a),c=b.display_title;if(c==undefined)c="Unknown title #"+a;var d=JSON.parse(b.body);b=htmlspecialchars(b.url);var e='<div class="document" id="document"';e+=' data-document-id="';e+=htmlspecialchars(a);e+='">';e+='<div class="row">';e+="<h3>";e+=htmlspecialchars(c);e+="</h3>";e+=" <pre><small>"+b+"</small></pre>";e+="</div>";e+='<div class="row">';for(var f in d){e+="<p>";e+=htmlspecialchars(d[f]);e+="</p>"}e+="</div>";e+='<div class="row">';
e+=" <pre><small>"+b+"</small></pre>";e+="</div>";e+="</div>";$("#documents").append(e);$(".excerpt").remove();dogeared_document_init_doc(a)}function dogeared_documents_cache(){var a=[];store.forEach(function(b,c){if(b.split("_")[0]=="dogeared"){c.cache_key=b;a.push(c)}});return a};var current_selection="",highlight_controls=0;function dogeared_document_init(){console.log("document init")}function dogeared_document_init_doc(a){console.log("document init (doc) "+a);window.onscroll=dogeared_document_on_scroll;(a=dogeared_whosonfirst_get(a))&&window.scrollTo(0,a);dogeared_document_bind_highlight_controls()}function dogeared_document_on_scroll(){var a=$("#document").attr("data-document-id");a&&dogeared_whosonfirst_set(a,window.scrollY)}
...
function dogeared_document_unbind_highlight_controls(){dogeared_omgwtf("unbind highlight controls");typeof window.ontouchend=="object"?$(document).unbind("selectionchange",dogeared_document_selected_selectionchange):$(document).unbind("mouseup",dogeared_document_selected_mouseup);$("#hint-highlight").unbind("click",dogeared_document_add_highlight)}
function dogeared_document_selected_mouseup(a){a=a.target;console.log("document: on mouseup on "+a.nodeName);if(a.nodeName=="BUTTON")$(a).attr("data-button-action")=="delete"?console.log("mouse up: ignore delete button"):dogeared_document_add_highlight();else{dogeared_document_hide_highlights_hint();$(".highlight").remove();window.getSelection().toString()!=""&&dogeared_document_show_highlights_hint()}}
function dogeared_document_selected_selectionchange(){dogeared_document_hide_highlights_hint();$(".highlight").remove();var a=window.getSelection().toString();if(a!=""){dogeared_document_show_highlights_hint();current_selection=a}}
function dogeared_document_add_highlight(){var a=$("#document").attr("data-document-id");if(!a){dogeared_feedback_error("Unable to determine what document I am looking at!");return false}var b=window.getSelection();b=current_selection?current_selection:b.toString();dogeared_cache_store_highlight({document_id:a,text:b})}function dogeared_document_show_highlights_hint(){$("#hint-highlight").show()}function dogeared_document_hide_highlights_hint(){$("#hint-highlight").hide()};var dogeared_highlights_flush=null;function dogeared_highlights_init(){window.addEventListener("online",function(){dogeared_highlights_on_online()});window.addEventListener("offline",function(){dogeared_highlights_on_offline()});dogeared_highlights_schedule_flush()}function dogeared_highlights_on_online(){dogeared_highlights_schedule_flush()}function dogeared_highlights_on_offline(){}
function dogeared_highlights_schedule_flush(){dogeared_highlights_flush&&clearTimeout(dogeared_highlights_flush);if(dogeared_network_is_online()){dogeared_highlights_flush_pending();if(dogeared_network_is_online())dogeared_highlights_flush=setTimeout(function(){dogeared_highlights_schedule_flush()},3E3)}}
function dogeared_highlights_flush_pending(){var a=dogeared_cache_highlights(),b=a.length;if(b!=0){dogeared_omgwtf("flush pending highlights ("+b+")");for(var c=0;c<b;c++)dogeared_highlights_flush_pending_single(a[c]);a=dogeared_cache_highlights();b=a.length;a=$("#has-pending");b?a.show():a.hide()}}
function dogeared_highlights_flush_pending_single(a){if(dogeared_network_is_online()){var b=a.cache_key;dogeared_omgwtf("flush pending highlight "+b);a.text?dogeared_api_call("dogeared.highlights.addHighlight",{document_id:a.document_id,text:a.text,created:a.created},function(){store.remove(b);dogeared_cache_highlights_status()}):store.remove(b)}}
...
function dogeared_notepad_on_online(){notepad_pending_sync.length&&dogeared_notepad_sync_notes(notepad_pending_sync)}function dogeared_notepad_add_note(){var a=prompt("What is the title of your note?");if(!a)return false;var b=Math.uuid(),c=dogeared_now(),d=dogeared_notepad_source();a={id:b,source_id:d,created:c,lastmodified:0,deleted:0,title:a,body:"This note left intentionally blank."};b=dogeared_notepad_key(a);store.set(b,a);dogeared_notepad_load_note(b)}
...
function dogeared_notepad_build_list(){var a=[];store.forEach(function(h,l){if(h.split("_")[0]=="notepad"){l.id||store.remove(h);parseInt(l.deleted)||a.push(l)}});a=dogeared_notepad_sort(a);for(var b=$("#note-editor"),c=$("#notes-list"),d="",e=a.length,f=0;f<e;f++){var g=a[f],j="notepad_"+g.id,k=g.title;g=g.source_id;d+="<li>";d+='<a href="#" class="note-item" data-note-key="'+j+'">'+k+"</a> ";d+=parseInt(g)?"<small>device id #"+g+"</small>":"<small>unknown source</small>";d+="</li>"}d+='<li><a href="#" id="note-add">Add a note</a></li>';
c.html(d);$("#note-add").click(function(){dogeared_notepad_add_note();return false});$(".note-item").click(function(){var h=$(this).attr("data-note-key");dogeared_notepad_load_note(h);return false});b.hide();c.show()}
...
function dogeared_notepad_sort(a){a=usort(a,function(b,c){if(b.lastmodified==c.lastmodified)return 0;return b.lastmodified>c.lastmodified?-1:1});return a=array_values(a)};function dogeared_cache_count_for_type(a){var b=0;store.forEach(function(c){if(c.split("_")[0]==a)b+=1});return b}function dogeared_cache_get_for_type(a){var b=[];store.forEach(function(c,d){if(c.split("_")[0]==a){d.cache_key=c;b.push(d)}});return b}function dogeared_cache_get(a,b){localforage.getItem(a,b)}function dogeared_cache_set(a,b,c){localforage.setItem(a,b,c)}function dogeared_cache_unset(a,b){localforage.removeItem(a,b)}
function dogeared_cache_keys(a,b){localforage.keys(function(c){for(var d=c.length,e=0;e<d;e++){var f=c[e],g=f.match(a);g&&b(f,g)}})}function dogeared_cache_store_document(a,b){var c="dogeared_"+a.id;b||(b=function(d){console.log(d)});dogeared_cache_set(c,a,b)}function dogeared_cache_load_document(a,b){dogeared_cache_get("dogeared_"+a,b)}
function dogeared_cache_remove_document(a,b){var c="dogeared_"+a;b||(b=function(){dogeared_feedback("Okay, that document ("+htmlspecialchars(a)+") has been removed from your reading list")});dogeared_cache_unset(c,b)}function dogeared_cache_store_highlight(a,b){var c=hex_md5(a.text);a.hash=c;var d="highlight_"+c;b||(b=function(){dogeared_omgwtf("stored "+d);$("#has-pending").show()});dogeared_cache_set(d,a,b)}
function dogeared_cache_schedule_pending_delete(a){dogeared_cache_set("dogeared_delete_"+a,true)}function dogeared_cache_remove_pending_delete(a){var b="dogeared_delete_"+a;dogeared_api_call("dogeared.documents.deleteDocument",{document_id:a},function(){dogeared_cache_unset(b)})}function dogeared_cache_list_pending_delete(){var a={};dogeared_cache_keys(function(b,c){a[c[1]]=true})}
function dogeared_cache_process_pending_delete(){dogeared_cache_keys(function(a,b){dogeared_cache_remove_pending_delete(b[1])})};function dogeared_cache_documents_store(a){dogeared_omgwtf("[warning] still calling dogeared_cache_documents_store");if(!store.enabled){console.log("localstorage is not enabled");return false}var b=a.id,c="dogeared_"+b;(b=store.set(c,a).id==b?1:0)||dogeared_feedback_error("Failed to cache "+c+", "+a.title);return b}function dogeared_cache_documents(){return dogeared_cache_get_for_type("dogeared")};function dogeared_cache_highlights_init(){dogeared_cache_highlights_status()}function dogeared_cache_highlights_store(a){if(!store.enabled){console.log("localstorage is not enabled");return false}var b=hex_md5(a.text);a.hash=b;b="highlight_"+b;console.log("store "+b);console.log(a);store.set(b,a);$("#has-pending").show();return true}function dogeared_cache_highlights_remove_key(a){store.remove(a)}
