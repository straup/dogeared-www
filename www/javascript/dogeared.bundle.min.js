...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents();a=dogeared_documents_sort(a);var b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var f=a[e],j=f.id,l=f.title?f.title:f.url;d[j]=e;var g='<div class="row excerpt">';g+='<h3><a href="#';g+='" class="load-doc" data-document-id="';g+=htmlspecialchars(j);g+='">';g+=htmlspecialchars(l);g+="</a>";
g+=" <small>offline cache</small>";g+="</h3>";g+="<p>";g+=htmlspecialchars(f.excerpt);g+=' <a href="#" class="load-doc" data-document-id="';g+=htmlspecialchars(j);g+='">...</a></p>';g+="</div>";c.append(g)}$(".load-doc").click(function(){var m=$(this).attr("data-document-id"),k=a[d[m]],n=k.title?k.title:k.url;k=JSON.parse(k.body);var h='<div class="row document" id="document"';h+=' data-document-id="';h+=htmlspecialchars(m);h+='">';h+="<h3>";h+=htmlspecialchars(n);h+="<small>offline cache</small>";
h+="</h3>";for(var o in k){h+="<p>";h+=htmlspecialchars(k[o]);h+="</p>"}h+="</document>";$(".excerpt").remove();c.append(h);return false});dogeared_document_init()}function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}
