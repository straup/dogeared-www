...
function dogeared_feedback_reset(){$("#feedback-general").html("")};function dogeared_api_call(a,b,c,e){var d=dogeared_api_endpoint();b.method=a;if(!b.access_token)if(a=dogeared_api_site_token())b.access_token=a;$.ajax({url:d,type:"POST",data:b,dataType:"json",success:function(f){c&&c(f)},error:function(f){if(f=function(g){if(g.responseText)try{return g=JSON.parse(g.responseText)}catch(j){console.log("Failed to parse response text");dogeared_feedback_error("The API is full of crazy-talk: Failed to parse response text")}else{console.log("Missing response text");console.log(g);
...
dogeared_cache_documents_store(b.document)});dogeared_feedback("Fetching your document, now...");$("#button-dogear").attr("disabled","disabled");return false});dogeared_network_is_online()||dogeared_extruder_hide_form()}function dogeared_extruder_on_online(){dogeared_extruder_show_form()}function dogeared_extruder_on_offline(){dogeared_extruder_hide_form()}function dogeared_extruder_show_form(){$("#fetch").show()}function dogeared_extruder_hide_form(){$("#fetch").hide()};function dogeared_documents_init(){dogeared_document_init_highlight_controls();window.addEventListener("offline",function(b){dogeared_documents_on_offline(b)});window.addEventListener("online",function(b){dogeared_documents_on_online(b)});$(".delete-document").click(function(){if(!confirm("Are you sure you want to remove this document?"))return false;var b=$(this).attr("data-document-id");dogeared_api_call("dogeared.documents.deleteDocument",{document_id:b},function(){$("#document-"+b).remove();store.remove("dogeared_"+
b);dogeared_feedback("Okay, that document has been removed from your reading list")})});if(dogeared_network_is_online())try{dogeared_documents_fill_cache();dogeared_documents_load_cache()}catch(a){dogeared_feedback_error("Oh no! There was a problem loading your documents: "+a);console.log(a)}else dogeared_documents_load_cache()}function dogeared_documents_on_online(){console.log("documents: online")}
function dogeared_documents_on_offline(){console.log("documents: offline");dogeared_documents_currently_reading()||dogeared_documents_load_cache()}
function dogeared_documents_load_cache(){console.log("load cache");window.scrollTo(0,0);var a=dogeared_cache_documents(),b=a.length;if(b==0)return false;a=dogeared_documents_sort(a);$(".excerpt").remove();$(".document").remove();var c=$("#documents");dogeared_abs_root_url();for(var e=0;e<b;e++){var d=a[e],f=d.id;if(f){var g=d.display_title;if(g==undefined)g="Unknown title #"+f;var j=dogeared_abs_root_url();j+="documents/"+f+"/";var h='<div class="row excerpt">';h+='<h3><a href="';h+=j;h+='" class="load-doc" data-document-id="';
h+=htmlspecialchars(f);h+='">';h+=htmlspecialchars(g);h+="</a>";h+=" <pre><small>"+htmlspecialchars(d.url)+"</small></pre>";h+="</h3>";h+="<div>";h+='<button class="btn btn-sm delete-document pull-right" data-document-id="';h+=f;h+='" data-button-action="delete">Delete</button>';h+="</div>";h+="</div>";c.append(h)}}$(".load-doc").click(function(){try{dogeared_network_is_online();var k=$(this).attr("data-document-id");dogeared_documents_load_doc(k)}catch(l){dogeared_feedback("Failed to load document because "+
l);console.log(l)}return false});dogeared_document_init()}function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}
function dogeared_documents_fill_cache(){for(var a={},b=dogeared_cache_documents(),c=b.length,e=0;e<c;e++)a[b[e].id]=1;dogeared_feedback("Fetching documents");dogeared_api_call("dogeared.documents.getList",{},function(d){dogeared_feedback("Storing documents");d=d.documents.document;var f=d.length;dogeared_documents_store_docs(d);dogeared_feedback("Cleaning up document cache");for(var g=0;g<f;g++){var j=d[g].document_id;a[j]&&delete a[j]}for(j in a)store.remove("dogeared_"+j);dogeared_feedback_reset()})}
function dogeared_documents_sort(a){a=usort(a,function(b,c){if(b.created==c.created)return 0;return b.created>c.created?-1:1});return a=array_values(a)}function dogeared_documents_currently_reading(){var a=$("#document").html()?1:0;console.log("currently reading: "+a);return a}
...
d+=" <pre><small>"+b+"</small></pre>";d+="</div>";d+="<div>";d+='<a href="">reading list</a>';d+="</div>";d+="</div>";$("#documents").append(d);$(".excerpt").remove();dogeared_document_init_doc(a)};var current_selection="";
...
function dogeared_notepad_build_list(){var a=[];store.forEach(function(k,l){if(k.split("_")[0]=="notepad"){l.id||store.remove(k);parseInt(l.deleted)||a.push(l)}});a=dogeared_notepad_sort(a);for(var b=$("#note-editor"),c=$("#notes-list"),e="",d=a.length,f=0;f<d;f++){var g=a[f],j="notepad_"+g.id,h=g.title;g=g.source_id;e+="<li>";e+='<a href="#" class="note-item" data-note-key="'+j+'">'+h+"</a> ";e+=parseInt(g)?"<small>device id #"+g+"</small>":"<small>unknown source</small>";e+="</li>"}e+='<li><a href="#" id="note-add">Add a note</a></li>';
c.html(e);$("#note-add").click(function(){dogeared_notepad_add_note();return false});$(".note-item").click(function(){var k=$(this).attr("data-note-key");dogeared_notepad_load_note(k);return false});b.hide();c.show()}
