...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents(),b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var f=0;f<b;f++){var e=a[f],j=e.id,l=e.title?e.title:e.url;d[j]=f;var g='<div class="row excerpt">';g+='<h3><a href="#';g+='" class="load-doc" data-document-id="';g+=htmlspecialchars(j);g+='">';g+=htmlspecialchars(l);g+="</a>";g+=" <small>offline cache</small>";
g+="</h3>";g+="<p>";g+=htmlspecialchars(e.excerpt);g+=' <a href="#" class="load-doc" data-document-id="';g+=htmlspecialchars(j);g+='">...</a></p>';g+="</div>";c.append(g)}$(".load-doc").click(function(){var m=$(this).attr("data-document-id"),k=a[d[m]],n=k.title?k.title:k.url;k=JSON.parse(k.body);var h='<div class="row document"';h+=' data-document-id="';h+=htmlspecialchars(m);h+='">';h+="<h3>";h+=htmlspecialchars(n);h+="<small>offline cache</small>";h+="</h3>";for(var o in k){h+="<p>";h+=htmlspecialchars(k[o]);
...
f;d=b.firstChild;)f=c.appendChild(d);range.insertNode(c);if(f){expandedSelRange.setEndAfter(f.previousSibling);a.removeAllRanges();a.addRange(expandedSelRange)}}}}
...
function dogeared_highlights_init_single(){$(".delete-highlight").click(function(){if(!confirm("Are you sure you want to delete this highlight?"))return false;var a={id:$(this).attr("data-highlight-id")};dogeared_api_call("dogeared.highlights.deleteHighlight",a,function(){var b=dogeared_abs_root_url()+"highlights/?deleted=1";location.href=b})})};var notepad_interval=null,notepad_pending_sync=[];function dogeared_notepad_init(){window.addEventListener("online",dogeared_notepad_on_online);dogeared_notepad_build_list()}function dogeared_notepad_on_online(){notepad_pending_sync.length&&dogeared_notepad_sync_notes(notepad_pending_sync)}
function dogeared_notepad_add_note(){var a=Math.uuid(),b=dogeared_now();a={id:a,created:b,lastmodified:0,deleted:0,title:"Untitled Note #"+a,body:"This note left intentionally blank."};b=dogeared_notepad_key(a);store.set(b,a);dogeared_notepad_load_note(b)}
function dogeared_notepad_load_note(a){var b=store.get(a),c=$("#notes-list"),d=$("#note-editor"),f=$("#note-editor-close"),e=$("#note-editor-delete");d.attr("data-note-id",a);$("#note-title").html(b.title);$("#note-body").html(b.body);f.click(function(){dogeared_notepad_close_note();return false});e.click(function(){if(!confirm("Are you sure you want to delete this note?"))return false;dogeared_notepad_delete_note();dogeared_notepad_build_list();return false});c.hide();d.show();notepad_interval=setInterval(function(){dogeared_notepad_save_note()},
...
function dogeared_notepad_build_list(){var a=$("#note-editor"),b=$("#notes-list"),c="";c+='<li><a href="#" id="note-add">Add a note</a></li>';store.forEach(function(d,f){if(d.split("_")[0]=="notepad")if(!f.deleted){var e=f.title,j=f.created;c+="<li>";c+='<a href="#" class="note-item" data-note-key="'+d+'">'+e+"</a> ";c+="<small>"+j+"</small>";c+="</li>"}});b.html(c);$("#note-add").click(function(){dogeared_notepad_add_note();return false});$(".note-item").click(function(){var d=$(this).attr("data-note-key");
dogeared_notepad_load_note(d);return false});a.hide();b.show()}function dogeared_notepad_sync_list(){store.forEach(function(a,b){a.split("_")[0]=="notepad"&&dogeared_notepad_sync_note(b)})}function dogeared_notepad_sync_pending_notes(a){for(id in a){var b=dogeared_notepad_key({id:id});b=store.get(b);dogeared_notepad_sync_note(b)}}
function dogeared_notepad_sync_note(a){if(!dogeared_network_is_online()){notepad_pending_sync[a.id]=true;return false}delete notepad_pending_sync[a.id];dogeared_api_call("dogeared.notepad.syncNote",a,function(b){console.log("sync ok");console.log(b)},function(b){console.log("sync NOT ok");console.log(b)});console.log("dogeared.notepad.syncNote");console.log(a)}
function dogeared_notepad_get_current_note(){var a=$("#note-editor").attr("data-note-id");if(a){var b=store.get(a);if(b)return b;else{dogeared_feedback_error("Failed to load "+a+" which is weird because I am trying to save it...");clearInterval(notepad_interval)}}else clearInterval(notepad_interval)}function dogeared_notepad_key(a){return"notepad_"+a.id};function dogeared_cache_count_for_type(a){var b=0;store.forEach(function(c){if(c.split("_")[0]==a)b+=1});return b}function dogeared_cache_get_for_type(a){var b=[];store.forEach(function(c,d){if(c.split("_")[0]==a){d.cache_key=c;b.push(d)}});return b};function dogeared_cache_documents_store(a){if(!store.enabled){console.log("localstorage is not enabled");return false}var b=a.lastmodified,c="dogeared_"+a.id,d=1;if(cache=store.get(c))if(cache.lastmodified>=b)d=0;d&&store.set(c,a)}function dogeared_cache_documents(){return dogeared_cache_get_for_type("dogeared")};function dogeared_cache_highlights_init(){dogeared_cache_highlights_status()}function dogeared_cache_highlights_store(a){if(!store.enabled){console.log("localstorage is not enabled");return false}var b=hex_md5(a.text);a.hash=b;b="highlight_"+b;if(cache=store.get(b))return true;store.set(b,a);return true}function dogeared_cache_highlights_remove_key(a){store.remove(a)}
