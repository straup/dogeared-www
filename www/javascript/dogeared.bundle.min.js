...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents(),b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),e={};dogeared_abs_root_url();for(var h=0;h<b;h++){var g=a[h],j=g.id,l=g.title?g.title:g.url;e[j]=h;var d='<div class="row excerpt">';d+='<h3><a href="#';d+='" class="load-doc" data-document-id="';d+=htmlspecialchars(j);d+='">';d+=htmlspecialchars(l);d+="</a>";d+=" <small>offline cache</small>";
d+="</h3>";d+="<p>";d+=htmlspecialchars(g.excerpt);d+=' <a href="#" class="load-doc" data-document-id="';d+=htmlspecialchars(j);d+='">...</a></p>';d+="</div>";c.append(d)}$(".load-doc").click(function(){var m=$(this).attr("data-document-id"),k=a[e[m]],n=k.title?k.title:k.url;k=JSON.parse(k.body);var f='<div class="row document"';f+=' data-document-id="';f+=htmlspecialchars(m);f+='">';f+="<h3>";f+=htmlspecialchars(n);f+="<small>offline cache</small>";f+="</h3>";for(var o in k){f+="<p>";f+=htmlspecialchars(k[o]);
f+="</p>"}f+="</document>";$(".excerpt").remove();c.append(f);return false});dogeared_document_init()}function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}
...
e,h;e=b.firstChild;)h=c.appendChild(e);range.insertNode(c);if(h){expandedSelRange.setEndAfter(h.previousSibling);a.removeAllRanges();a.addRange(expandedSelRange)}}}}
