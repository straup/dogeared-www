...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents();a=dogeared_documents_sort(a);var b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var f=a[e],h=f.id,k=f.display_title;if(k==undefined)k="Unknown title #"+h;d[h]=e;var o=dogeared_abs_root_url();o+="documents/"+h+"/";var j='<div class="row excerpt">';j+='<h3><a href="';j+=o;j+='" class="load-doc" data-document-id="';
j+=htmlspecialchars(h);j+='">';j+=htmlspecialchars(k);j+="</a>";j+=" <pre><small>"+htmlspecialchars(f.url)+"</small></pre>";j+="</h3>";j+="<div>";j+='<button class="btn btn-sm delete-document" data-document-id="';j+=h;j+='" data-button-action="delete">Delete</button>';j+="</div>";j+="</div>";c.append(j)}$(".load-doc").click(function(){if(dogeared_network_is_online())return true;var m=$(this).attr("data-document-id"),l=a[d[m]],n=l.display_title;if(n==undefined)n="Unknown title #"+m;var p=JSON.parse(l.body);
l=htmlspecialchars(l.url);var g='<div class="row document" id="document"';g+=' data-document-id="';g+=htmlspecialchars(m);g+='">';g+="<h3>";g+=htmlspecialchars(n);g+=" <pre><small>"+l+"</small></pre>";g+="</h3>";g+="</div>";g+="<div>";for(var q in p){g+="<p>";g+=htmlspecialchars(p[q]);g+="</p>"}g+="</div>";g+="<div>";g+=" <pre><small>"+l+"</small></pre>";g+="</div>";g+="</document>";$(".excerpt").remove();c.append(g);return false});dogeared_document_init()}
function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}
function dogeared_documents_fill_cache(){for(var a={},b=dogeared_cache_documents(),c=b.length,d=0;d<c;d++)a[b[d].id]=1;dogeared_feedback("Fetching documents");dogeared_api_call("dogeared.documents.getList",{},function(e){dogeared_feedback("Storing documents");e=e.documents.document;var f=e.length;dogeared_documents_store_docs(e);dogeared_feedback("Cleaning up document cache");for(var h=0;h<f;h++){var k=e[h].document_id;a[k]&&delete a[k]}for(k in a){e="dogeared_"+k;console.log("delete "+e);store.remove(e)}dogeared_feedback_reset()})}
...
function dogeared_notepad_build_list(){var a=$("#note-editor"),b=$("#notes-list"),c="";c+='<li><a href="#" id="note-add">Add a note</a></li>';store.forEach(function(d,e){if(d.split("_")[0]=="notepad"){e.id||store.remove(d);if(!parseInt(e.deleted)){var f=e.title,h=e.source_id;c+="<li>";c+='<a href="#" class="note-item" data-note-key="'+d+'">'+f+"</a> ";c+=parseInt(h)?"<small>device id #"+h+"</small>":"<small>unknown source</small>";c+="</li>"}}});b.html(c);$("#note-add").click(function(){dogeared_notepad_add_note();
