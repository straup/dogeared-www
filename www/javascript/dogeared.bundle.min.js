...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents();a=dogeared_documents_sort(a);var b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var g=a[e],j=g.id,l=g.display_title;if(l==undefined)l="Unknown title #"+j;d[j]=e;var o=dogeared_abs_root_url();o+="documents/"+j+"/";var h='<div class="row excerpt">';h+='<h3><a href="';h+=o;h+='" class="load-doc" data-document-id="';
h+=htmlspecialchars(j);h+='">';h+=htmlspecialchars(l);h+="</a>";h+=" <pre><small>"+htmlspecialchars(g.url)+"</small></pre>";h+="</h3>";h+="<div>";h+='<button class="btn btn-sm delete-document" data-document-id="';h+=j;h+='" data-button-action="delete">Delete</button>';h+="</div>";h+="</div>";c.append(h)}$(".load-doc").click(function(){if(dogeared_network_is_online())return true;var m=$(this).attr("data-document-id"),k=a[d[m]],n=k.display_title;if(n==undefined)n="Unknown title #"+m;var p=JSON.parse(k.body);
k=htmlspecialchars(k.url);var f='<div class="row document" id="document"';f+=' data-document-id="';f+=htmlspecialchars(m);f+='">';f+="<h3>";f+=htmlspecialchars(n);f+=" <pre><small>"+k+"</small></pre>";f+="</h3>";f+="</div>";f+="<div>";for(var q in p){f+="<p>";f+=htmlspecialchars(p[q]);f+="</p>"}f+="</div>";f+="<div>";f+=" <pre><small>"+k+"</small></pre>";f+="</div>";f+="</document>";$(".excerpt").remove();c.append(f);return false});dogeared_document_init()}
function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}function dogeared_documents_fill_cache(){dogeared_api_call("dogeared.documents.getList",{},function(a){dogeared_documents_store_docs(a.documents.document)})}
...
function dogeared_notepad_build_list(){var a=$("#note-editor"),b=$("#notes-list"),c="";c+='<li><a href="#" id="note-add">Add a note</a></li>';store.forEach(function(d,e){if(d.split("_")[0]=="notepad"){e.id||store.remove(d);if(!parseInt(e.deleted)){var g=e.title,j=e.source_id;c+="<li>";c+='<a href="#" class="note-item" data-note-key="'+d+'">'+g+"</a> ";c+=parseInt(j)?"<small>device id #"+j+"</small>":"<small>unknown source</small>";c+="</li>"}}});b.html(c);$("#note-add").click(function(){dogeared_notepad_add_note();
