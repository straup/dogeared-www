...
h+=g;h+='" data-button-action="delete">Delete</button>';h+="</div>";h+="</div>";e.append(h)}}$(".load-doc").click(function(){try{dogeared_network_is_online();var l=$(this).attr("data-document-id");dogeared_documents_load_doc(l)}catch(m){dogeared_feedback("Failed to load document because "+m);console.log(m)}return false});dogeared_document_init()}
function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){b=b.document;var c=dogeared_cache_documents_store(b);dogeared_omgwtf("store "+b.display_title+" ("+b.id+") "+c);dogeared_documents_fill_cache_decr()},function(){dogeared_documents_fill_cache_decr()});dogeared_documents_fill_cache_incr()}
