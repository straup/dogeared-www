...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents(),b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var g=0;g<b;g++){var f=a[g],j=f.id,l=f.title?f.title:f.url;d[j]=g;var e='<div class="row excerpt">';e+='<h3><a href="#';e+='" class="load-doc" data-document-id="';e+=htmlspecialchars(j);e+='">';e+=htmlspecialchars(l);e+="</a>";e+=" <small>offline cache</small>";
e+="</h3>";e+="<p>";e+=htmlspecialchars(f.excerpt);e+=' <a href="#" class="load-doc" data-document-id="';e+=htmlspecialchars(j);e+='">...</a></p>';e+="</div>";c.append(e)}$(".load-doc").click(function(){var m=$(this).attr("data-document-id"),k=a[d[m]],n=k.title?k.title:k.url;k=JSON.parse(k.body);var h='<div class="row document"';h+=' data-document-id="';h+=htmlspecialchars(m);h+='">';h+="<h3>";h+=htmlspecialchars(n);h+="<small>offline cache</small>";h+="</h3>";for(var o in k){h+="<p>";h+=htmlspecialchars(k[o]);
...
g;d=b.firstChild;)g=c.appendChild(d);range.insertNode(c);if(g){expandedSelRange.setEndAfter(g.previousSibling);a.removeAllRanges();a.addRange(expandedSelRange)}}}}
...
function dogeared_highlights_init_single(){$(".delete-highlight").click(function(){if(!confirm("Are you sure you want to delete this highlight?"))return false;var a={id:$(this).attr("data-highlight-id")};dogeared_api_call("dogeared.highlights.deleteHighlight",a,function(){var b=dogeared_abs_root_url()+"highlights/?deleted=1";location.href=b})})};var notepad_interval=null;function dogeared_notepad_init(){dogeared_notepad_build_list()}function dogeared_notepad_add_note(){var a=Math.uuid(),b="notepad_"+a;store.set(b,{title:"Untitled Note #"+a,body:"This note left intentionally blank."});dogeared_notepad_load_note(b)}
function dogeared_notepad_load_note(a){var b=store.get(a);console.log(b);var c=$("#notes-list"),d=$("#note-editor"),g=$("#note-editor-close");d.attr("data-note-id",a);$("#note-title").html(b.title);$("#note-body").html(b.body);g.click(function(){dogeared_notepad_close_note();return false});c.hide();d.show();notepad_interval=setInterval(function(){dogeared_notepad_save_note()},5E3);console.log("set interval "+notepad_interval)}
function dogeared_notepad_save_note(){var a=$("#note-editor").attr("data-note-id");if(a){console.log("save "+a);var b=$("#note-title");b=b.html();var c=$("#note-body");c=c.html();store.set(a,{title:b,body:c})}else{console.log("clear interval "+notepad_interval);clearInterval(notepad_interval)}}
