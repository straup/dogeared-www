...
return false});$(".note-item").click(function(){var d=$(this).attr("data-note-key");dogeared_notepad_load_note(d);return false});a.hide();b.show()}function dogeared_notepad_sync_list(){store.forEach(function(a,b){if(a.split("_")[0]=="notepad")b["delete"]?store.remove(a):dogeared_notepad_sync_note(b)})}function dogeared_notepad_sync_pending_notes(a){for(id in a){var b=dogeared_notepad_key({id:id});b=store.get(b);dogeared_notepad_sync_note(b)}}
function dogeared_notepad_sync_note(a){var b=a.id;if(!dogeared_network_is_online()){notepad_pending_sync[b]=true;return false}delete notepad_pending_sync[b];notepad_inprocess_sync[b]=true;a={note:JSON.stringify(a)};dogeared_api_call("dogeared.notepad.syncNote",a,function(c){c=c.note;var d=c.id,e=dogeared_notepad_key(c);store.set(e,c);notepad_inprocess_sync[d]=false},function(){notepad_inprocess_sync[b]=false})}
