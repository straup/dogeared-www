...
function dogeared_api_endpoint(){return document.body.getAttribute("data-api-endpoint")}function dogeared_api_site_token(){return document.body.getAttribute("data-api-site-token")}function dogeared_api_hints_ok(){$("#api-dispatch").hide();$("#api-ok").show();setTimeout(function(){$("#api-ok").hide()},5E3)}function dogeared_api_hints_error(){$("#api-error").show()}function dogeared_api_hints_dispatch(){$("#api-dispatch").show()};function dogeared_extruder_init(){window.addEventListener("offline",function(a){dogeared_extruder_on_offline(a)});window.addEventListener("online",function(a){dogeared_extruder_on_online(a)});$("#fetch").submit(function(){var a=$("#url");a=a.val();dogeared_api_call("dogeared.documents.addDocument",{url:a},function(b){var c=b.document.id;c='Okay! <a href="'+(dogeared_abs_root_url()+"documents/"+c+"/")+'">That document</a> is ready to be dogeared.';dogeared_feedback_ok(c,60);$("#button-dogear").removeAttr("disabled");
...
d+=" <pre><small>"+b+"</small></pre>";d+="</div>";d+="<div>";d+='<a href="">reading list</a>';d+="</div>";d+="</div>";$("#documents").append(d);$(".excerpt").remove();dogeared_document_init_doc(a)}function dogeared_documents_cache(){var a=[];store.forEach(function(b,c){if(b.split("_")[0]=="dogeared"){c.cache_key=b;a.push(c)}});return a}
