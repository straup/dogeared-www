...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents(),b=a.length;if(b==0)return false;a=dogeared_documents_sort(a);a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var g=a[e],h=g.id,k=g.display_title;if(k==undefined)k="Unknown title #"+h;d[h]=e;var o=dogeared_abs_root_url();o+="documents/"+h+"/";var j='<div class="row excerpt">';j+='<h3><a href="';j+=o;j+='" class="load-doc" data-document-id="';
j+=htmlspecialchars(h);j+='">';j+=htmlspecialchars(k);j+="</a>";j+=" <pre><small>"+htmlspecialchars(g.url)+"</small></pre>";j+="</h3>";j+="<div>";j+='<button class="btn btn-sm delete-document pull-right" data-document-id="';j+=h;j+='" data-button-action="delete">Delete</button>';j+="</div>";j+="</div>";c.append(j)}$(".load-doc").click(function(){dogeared_network_is_online();var m=$(this).attr("data-document-id"),l=a[d[m]],n=l.display_title;if(n==undefined)n="Unknown title #"+m;var p=JSON.parse(l.body);
l=htmlspecialchars(l.url);var f='<div class="document" id="document"';f+=' data-document-id="';f+=htmlspecialchars(m);f+='">';f+='<div class="row">';f+="<h3>";f+=htmlspecialchars(n);f+=" <pre><small>"+l+"</small></pre>";f+="</h3>";f+="</div>";f+='<div class="row">';for(var q in p){f+="<p>";f+=htmlspecialchars(p[q]);f+="</p>"}f+="</div>";f+='<div class="row">';f+=" <pre><small>"+l+"</small></pre>";f+="</div>";f+="<div>";f+='<a href="">reading list</a>';f+="</div>";f+="</div>";$(".excerpt").remove();
...
function dogeared_documents_fill_cache(){for(var a={},b=dogeared_cache_documents(),c=b.length,d=0;d<c;d++)a[b[d].id]=1;dogeared_feedback("Fetching documents");dogeared_api_call("dogeared.documents.getList",{},function(e){dogeared_feedback("Storing documents");e=e.documents.document;var g=e.length;dogeared_documents_store_docs(e);dogeared_feedback("Cleaning up document cache");for(var h=0;h<g;h++){var k=e[h].document_id;a[k]&&delete a[k]}for(k in a){e="dogeared_"+k;console.log("delete "+e);store.remove(e)}dogeared_feedback_reset()})}
...
function dogeared_notepad_load_note(a){var b=store.get(a),c=$("#notes-list"),d=$("#note-editor"),e=$("#note-editor-close"),g=$("#note-editor-delete");d.attr("data-note-id",a);$("#note-title").html(b.title);$("#note-body").html(b.body);e.click(function(){dogeared_notepad_close_note();return false});g.click(function(){if(!confirm("Are you sure you want to delete this note?"))return false;dogeared_notepad_delete_note();dogeared_notepad_build_list();return false});c.hide();d.show();notepad_interval=setInterval(function(){dogeared_notepad_save_note()},
...
function dogeared_notepad_build_list(){var a=$("#note-editor"),b=$("#notes-list"),c="";c+='<li><a href="#" id="note-add">Add a note</a></li>';store.forEach(function(d,e){if(d.split("_")[0]=="notepad"){e.id||store.remove(d);if(!parseInt(e.deleted)){var g=e.title,h=e.source_id;c+="<li>";c+='<a href="#" class="note-item" data-note-key="'+d+'">'+g+"</a> ";c+=parseInt(h)?"<small>device id #"+h+"</small>":"<small>unknown source</small>";c+="</li>"}}});b.html(c);$("#note-add").click(function(){dogeared_notepad_add_note();
return false});$(".note-item").click(function(){var d=$(this).attr("data-note-key");dogeared_notepad_load_note(d);return false});a.hide();b.show()}
