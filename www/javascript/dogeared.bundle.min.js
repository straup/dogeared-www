...
d+=" <pre><small>"+b+"</small></pre>";d+="</div>";d+="</div>";$("#documents").append(d);$(".excerpt").remove();dogeared_document_init_doc(a)}function dogeared_documents_cache(){var a=[];store.forEach(function(b,c){if(b.split("_")[0]=="dogeared"){c.cache_key=b;a.push(c)}});return a}
function dogeared_documents_store(a){if(!store.enabled){console.log("localstorage is not enabled");return false}var b=a.id,c="dogeared_"+b;try{var e=store.set(c,a)}catch(d){dogeared_feedback_error("Failed to cache "+c+", "+a.title+" because "+d);return 0}(b=e.id==b?1:0)||dogeared_feedback_error("Failed to cache "+c+", "+a.title);return b};var current_selection="",highlight_controls=0;function dogeared_document_init(){console.log("document init")}function dogeared_document_init_doc(a){console.log("document init (doc) "+a);window.onscroll=dogeared_document_on_scroll;(a=dogeared_whosonfirst_get(a))&&window.scrollTo(0,a);dogeared_document_bind_highlight_controls()}function dogeared_document_on_scroll(){var a=$("#document").attr("data-document-id");a&&dogeared_whosonfirst_set(a,window.scrollY)}
...
function dogeared_notepad_sort(a){a=usort(a,function(b,c){if(b.lastmodified==c.lastmodified)return 0;return b.lastmodified>c.lastmodified?-1:1});return a=array_values(a)};function dogeared_cache_count_for_type(a){var b=0;store.forEach(function(c){if(c.split("_")[0]==a)b+=1});return b}function dogeared_cache_get_for_type(a){var b=[];store.forEach(function(c,e){if(c.split("_")[0]==a){e.cache_key=c;b.push(e)}});return b}function dogeared_cache_get(a,b){localforage.getItem(a,b)}function dogeared_cache_set(a,b,c){localforage.setItem(a,b,c)}function dogeared_cache_unset(a,b){localforage.removeItem(a,b)}
function dogeared_cache_keys(a,b){localforage.keys(function(c){for(var e=c.length,d=0;d<e;d++){var f=c[d],g=f.match(a);g&&b(f,g)}})}function dogeared_cache_remove_document(a,b){var c="dogeared_"+a;b||dogeared_feedback("Okay, that document ("+htmlspecialchars(a)+") has been removed from your reading list");dogeared_cache_unset(c,b)}
function dogeared_cache_store_highlight(a,b){var c=hex_md5(a.text);a.hash=c;c="highlight_"+c;if(!b){dogeared_omgwtf("stored "+c);$("#has-pending").show()}dogeared_cache_set(c,a,b)}function dogeared_cache_schedule_pending_delete(a){dogeared_cache_set("dogeared_delete_"+a,true)}function dogeared_cache_remove_pending_delete(a){var b="dogeared_delete_"+a;dogeared_api_call("dogeared.documents.deleteDocument",{document_id:a},function(){dogeared_cache_unset(b)})}
