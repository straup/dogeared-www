...
function dogeared_documents_load_index(){if(dogeared_network_is_online())try{dogeared_documents_fill_cache();dogeared_documents_load_cache()}catch(a){dogeared_feedback_error("Oh no! There was a problem loading your documents: "+a);console.log(a)}else dogeared_documents_load_cache()}
function dogeared_documents_init_delete_controls(){dogeared_omgwtf("init documents delete controls");var a=function(b){if(!confirm("Are you sure you want to remove this document?"))return false;var c=b.attr("data-document-id");if(!c){alert("Unable to determine document ID");return false}dogeared_omgwtf("delete document "+c);if(!dogeared_network_is_online()){(b=store.get("dogeared_to_delete"))||(b={});b[c]=true;store.set("dogeared_to_delete",b);$("#dogeared-document-"+c).remove();dogeared_feedback_modal("Okay. That document has been queued for deletion.");
return false}dogeared_api_call("dogeared.documents.deleteDocument",{document_id:c},function(){$("#document-"+c).remove();store.remove("dogeared_"+c);dogeared_feedback("Okay, that document has been removed from your reading list");dogeared_documents_load_index()})};$(".delete-document").click(function(){dogeared_omgwtf("click to delete");try{var b=$(this);a(b)}catch(c){dogeared_feedback_error("Huh. Deleting your document failed because: "+c)}})}
...
function dogeared_documents_load_cache(){dogeared_omgwtf("load documents cache");window.scrollTo(0,0);var a=store.get("dogeared_to_delete"),b=dogeared_cache_documents(),c=b.length;if(c==0)return false;b=dogeared_documents_sort(b);$(".excerpt").remove();$(".document").remove();var e=$("#documents");dogeared_abs_root_url();for(var d=0;d<c;d++){var h=b[d],f=h.id;if(f)if(a&&a[f])dogeared_omgwtf("skipping document "+f+" because it is set to be deleted");else{var j=h.display_title;if(j==undefined)j="Unknown title #"+
f;var l=dogeared_abs_root_url();l+="documents/"+f+"/";var g='<div class="row excerpt"';g+=' id="dogeared-document-'+f+'"';g+=">";g+='<h3><a href="';g+=l;g+='" class="load-doc" data-document-id="';g+=htmlspecialchars(f);g+='">';g+=htmlspecialchars(j);g+="</a>";g+=" <pre><small>"+htmlspecialchars(h.url)+"</small></pre>";g+="</h3>";g+="<div>";g+='<button class="btn btn-sm delete-document pull-right" data-document-id="';g+=f;g+='" data-button-action="delete">Delete</button>';g+="</div>";g+="</div>";e.append(g)}}$(".load-doc").click(function(){try{dogeared_network_is_online();
var k=$(this).attr("data-document-id");dogeared_documents_load_doc(k)}catch(m){dogeared_feedback("Failed to load document because "+m);console.log(m)}return false});dogeared_document_init()}function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}
function dogeared_documents_fill_cache(){for(var a={},b=dogeared_cache_documents(),c=b.length,e=0;e<c;e++)a[b[e].id]=1;dogeared_feedback("Fetching documents");dogeared_api_call("dogeared.documents.getList",{},function(d){dogeared_feedback("Storing documents");d=d.documents.document;var h=d.length;dogeared_documents_store_docs(d);dogeared_feedback("Cleaning up document cache");for(var f=0;f<h;f++){var j=d[f].document_id;a[j]&&delete a[j]}for(j in a)store.remove("dogeared_"+j);dogeared_feedback_reset()})}
...
function dogeared_documents_load_doc(a){var b=store.get("dogeared_"+a),c=b.display_title;if(c==undefined)c="Unknown title #"+a;var e=JSON.parse(b.body);b=htmlspecialchars(b.url);var d='<div class="document" id="document"';d+=' data-document-id="';d+=htmlspecialchars(a);d+='">';d+='<div class="row">';d+="<h3>";d+=htmlspecialchars(c);d+="</h3>";d+=" <pre><small>"+b+"</small></pre>";d+="</div>";d+='<div class="row">';for(var h in e){d+="<p>";d+=htmlspecialchars(e[h]);d+="</p>"}d+="</div>";d+='<div class="row">';
d+=" <pre><small>"+b+"</small></pre>";d+="</div>";d+="<div>";d+='<a href="">reading list</a>';d+="</div>";d+="</div>";$("#documents").append(d);$(".excerpt").remove();dogeared_document_init_doc(a)};var current_selection="";function dogeared_document_init(){console.log("document init")}function dogeared_document_init_doc(a){console.log("document init (doc) "+a);dogeared_document_init_highlight_controls();window.onscroll=dogeared_document_on_scroll;(a=dogeared_whosonfirst_get(a))&&window.scrollTo(0,a)}function dogeared_document_on_scroll(){var a=$("#document").attr("data-document-id");a&&dogeared_whosonfirst_set(a,window.scrollY)}
function dogeared_document_init_highlight_controls(){if(typeof window.ontouchend=="object"){console.log("init touch based highlight controls");$(document).bind("selectionchange",dogeared_document_selected_selectionchange)}else{console.log("init mouse based highlight controls");$(document).bind("mouseup",dogeared_document_selected_mouseup)}}
...
function dogeared_notepad_load_note(a){var b=store.get(a),c=$("#notes-list"),e=$("#note-editor"),d=$("#note-editor-close"),h=$("#note-editor-delete");e.attr("data-note-id",a);$("#note-title").html(b.title);$("#note-body").html(b.body);d.click(function(){dogeared_notepad_close_note();return false});h.click(function(){if(!confirm("Are you sure you want to delete this note?"))return false;dogeared_notepad_delete_note();dogeared_notepad_build_list();return false});c.hide();e.show();notepad_interval=setInterval(function(){dogeared_notepad_save_note()},
...
function dogeared_notepad_build_list(){var a=[];store.forEach(function(g,k){if(g.split("_")[0]=="notepad"){k.id||store.remove(g);parseInt(k.deleted)||a.push(k)}});a=dogeared_notepad_sort(a);for(var b=$("#note-editor"),c=$("#notes-list"),e="",d=a.length,h=0;h<d;h++){var f=a[h],j="notepad_"+f.id,l=f.title;f=f.source_id;e+="<li>";e+='<a href="#" class="note-item" data-note-key="'+j+'">'+l+"</a> ";e+=parseInt(f)?"<small>device id #"+f+"</small>":"<small>unknown source</small>";e+="</li>"}e+='<li><a href="#" id="note-add">Add a note</a></li>';
c.html(e);$("#note-add").click(function(){dogeared_notepad_add_note();return false});$(".note-item").click(function(){var g=$(this).attr("data-note-key");dogeared_notepad_load_note(g);return false});b.hide();c.show()}
