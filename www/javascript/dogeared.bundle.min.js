...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents();a=dogeared_documents_sort(a);var b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var f=a[e],h=f.id,l=f.display_title;if(l==undefined)l="Unknown title #"+h;d[h]=e;var o=dogeared_abs_root_url();o+="documents/"+h+"/";var g='<div class="row excerpt">';g+='<h3><a href="';g+=o;g+='" class="load-doc" data-document-id="';
g+=htmlspecialchars(h);g+='">';g+=htmlspecialchars(l);g+="</a>";g+=" <small>"+htmlspecialchars(f.url)+"</small>";g+="</h3>";g+="<div>";g+='<button class="btn btn-sm delete-document" data-document-id="';g+=h;g+='" data-button-action="delete">Delete</button>';g+="</div>";g+="</div>";c.append(g)}$(".load-doc").click(function(){if(dogeared_network_is_online())return true;var m=$(this).attr("data-document-id"),k=a[d[m]],n=k.display_title;if(n==undefined)n="Unknown title #"+m;k=JSON.parse(k.body);var j=
'<div class="row document" id="document"';j+=' data-document-id="';j+=htmlspecialchars(m);j+='">';j+="<h3>";j+=htmlspecialchars(n);j+="<small>offline cache</small>";j+="</h3>";for(var p in k){j+="<p>";j+=htmlspecialchars(k[p]);j+="</p>"}j+="</document>";$(".excerpt").remove();c.append(j);return false});dogeared_document_init()}
