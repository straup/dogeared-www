...
function dogeared_documents_load_cache(){console.log("load cache");var a=dogeared_cache_documents();a=dogeared_documents_sort(a);var b=a.length;if(b==0)return false;a.reverse();$(".excerpt").remove();$(".document").remove();var c=$("#documents"),d={};dogeared_abs_root_url();for(var e=0;e<b;e++){var f=a[e],j=f.id,k=f.display_title;if(k==undefined)k="Unknown title #"+j;console.log("title is "+k);d[j]=e;var g='<div class="row excerpt">';g+='<h3><a href="#';g+='" class="load-doc" data-document-id="';
g+=htmlspecialchars(j);g+='">';g+=htmlspecialchars(k);g+="</a>";g+=" <small>offline cache</small>";g+="</h3>";g+="<p>";g+=htmlspecialchars(f.excerpt);g+=' <a href="#" class="load-doc" data-document-id="';g+=htmlspecialchars(j);g+='">...</a></p>';g+="</div>";c.append(g)}$(".load-doc").click(function(){var m=$(this).attr("data-document-id"),l=a[d[m]],n=l.display_title;if(n==undefined)n="Unknown title #"+m;l=JSON.parse(l.body);var h='<div class="row document" id="document"';h+=' data-document-id="';
h+=htmlspecialchars(m);h+='">';h+="<h3>";h+=htmlspecialchars(n);h+="<small>offline cache</small>";h+="</h3>";for(var o in l){h+="<p>";h+=htmlspecialchars(l[o]);h+="</p>"}h+="</document>";$(".excerpt").remove();c.append(h);return false});dogeared_document_init()}function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}
function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}function dogeared_documents_fill_cache(){dogeared_api_call("dogeared.documents.getList",{},function(a){dogeared_documents_store_docs(a.documents.document)})}function dogeared_documents_sort(a){a=usort(a,function(b,c){if(b.created==c.created)return 0;return b.created<c.created?-1:1});return a=array_values(a)};var current_selection="";
