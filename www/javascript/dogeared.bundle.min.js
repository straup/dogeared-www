...
JSON.parse(m.body),q=htmlspecialchars(m.url),f='<div class="document" id="document"';f+=' data-document-id="';f+=htmlspecialchars(l);f+='">';f+='<div class="row">';f+="<h3>";f+=htmlspecialchars(o);f+="</h3>";f+=" <pre><small>"+q+"</small></pre>";f+="</div>";f+='<div class="row">';for(var r in p){f+="<p>";f+=htmlspecialchars(p[r]);f+="</p>"}f+="</div>";f+='<div class="row">';f+=" <pre><small>"+q+"</small></pre>";f+="</div>";f+="<div>";f+='<a href="">reading list</a>';f+="</div>";f+="</div>";c.append(f);
$(".excerpt").remove();m.pos&&window.scrollto(0,m.pos);return false});dogeared_document_init()}function dogeared_documents_get_text(a){a=a.document_id;if(a=="0")return false;dogeared_api_call("dogeared.documents.getInfo",{document_id:a},function(b){dogeared_cache_documents_store(b.document)})}function dogeared_documents_store_docs(a){for(i in a)dogeared_documents_get_text(a[i])}
...
function dogeared_document_init(){console.log("document init");dogeared_document_init_highlight_controls();$(".delete-document").click(function(){if(!dogeared_network_is_online()){alert("Offline deletes are not supported yet.");return false}if(!confirm("Are you sure you want to remove this document?"))return false;var a=$(this).attr("data-document-id");dogeared_api_call("dogeared.documents.deleteDocument",{document_id:a},function(){store.remove("dogeared_"+a);var b=dogeared_abs_root_url()+"documents/";
location.href=b})});window.onscroll=dogeared_document_on_scroll}function dogeared_document_on_scroll(){var a=$("#document").attr("data-document-id");console.log("scroll "+a);if(a){a="dogeared_"+a;var b=store.get(a);b.pos=window.scrollY;store.set(a,b);console.log(a+":"+window.scrollY)}}
